...
router.post('/ask', optionalAuth, enforceLimits, enforceGuestDemoLimit, async (req, res) => {
    try {
        ...
        const isExplicitBmuLawQuestion = /\b(bmu\s+law|bayelsa\s+medical\s+university\s+yenagoa\s+law|university\s+law|under\s+the\s+law|according\s+to\s+the\s+law|statute|statutes|section\s+\d+)\b/i.test(normalizedQuestion);
