...
Invoke-Remote "cd $AppDir && SMOKE_TEST_STRICT=true SMOKE_TEST_URL=`"$HealthBaseUrl/api/health`" SMOKE_TEST_ADVISOR_URL=`"$HealthBaseUrl/api/advisor/health`" npm test && npm run test:structured-facts && npm run test:advisor-policy && npm run test:stt-protection && npm run test:course-data-quality && npm run test:advisor-golden"
